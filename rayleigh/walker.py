...
class ConstraintWalker(ast.NodeVisitor):
    ...
    def visit_Assign(self, node: ast.Assign) -> None:
        ...
        expression_text = ast.unparse(node.value)
    ...
    def visit_For(self, node: ast.For) -> None:
        # V1 intentionally does not infer loop-carried state.
        ...
        if isinstance(target, ast.Name):
            ...
            if not (
                ...
                self._add_equality(
                    ...
                    chain=(
                        f"{name} = "
                        f"{expression_text or self._format_expr(value)}",
                    ),
                )
            ...
            if prior:
                ...
                self._add_equality(
                    lhs,
                    DimensionExpr.concrete(dimension),
                    line,
                    reason,
                    ConstraintKind.PRIOR,
                    chain=(
                        f"{name} → {dimension.format()}",
                        f"reason: {reason}",
                    ),
                )
            ...
            target,
            (ast.Tuple, ast.List),
        ) and isinstance(value, DimensionExpr):
            # V1 records no destructuring semantics.
            for elt in target.elts:
                if isinstance(elt, ast.Name):
                    self.variables.add(elt.id)

    @staticmethod
    def _format_expr(expr: DimensionExpr) -> str:
        return str(expr)
    ...
    def _expr(self, node: ast.AST) -> DimensionExpr:
        ...
        if isinstance(node, ast.Name):
            ...
            self._add_equality(
                ...
                chain=(
                    "both branches must have equal dimensions",
                ),
            )
            ...
            # Cross-module attribute inference is explicitly out of scope.
            result = DimensionExpr.unknown(
                ast.unparse(node)
            )
    ...
    def _binop(self, node: ast.BinOp) -> DimensionExpr:
        ...
        # Floor division has the same dimensional algebra as division.
    ...
    def _call(self, node: ast.Call) -> DimensionExpr:
        ...
        if name in {
            ...
            if args:
                ...
                self._add_equality(
                    ...
                    f"{name} argument",
                    chain=(
                        f"{name} argument must be dimensionless",
                    ),
                )
        ...
        if name in {"min", "max"} and args:
            for arg in args[1:]:
                self._add_equality(
                    ...
                    f"{name} arguments",
                    chain=(
                        f"{name} arguments must have equal dimensions",
                    ),
                )
        ...
        return DimensionExpr.unknown(
            f"call:{name or '<call>'}@{line}"
        )

    @staticmethod
    def _is_plain_numeric_value(
        value: DimensionExpr,
    ) -> bool:
        return (
    ...
    def _constant_expr(
        ...
        if (
            ...
            if fingerprint:
                self.constraints.append(
                    Constraint(
                        left=DimensionExpr.unknown(
                            f"const:{node.value}"
                        ),
                        right=DimensionExpr.concrete(
                        ...
                        chain=(
                            ...
                            (
                                f"{fingerprint.name} → "
                                f"{fingerprint.dimension.format()}"
                            ),
                        ),
                    )
                )
        ...
        return DimensionExpr.unknown(
            f"literal@{getattr(node, 'lineno', 0)}"
        )
